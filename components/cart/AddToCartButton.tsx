...
import { getCoursePlayerHref, getCourseSlug, type CourseSlug } from "@/lib/course-slug";
...
export function AddToCartButton({
  ...
  const courseSlug = getCourseSlug(course.slug);
  const coursePlayerHref = getCoursePlayerHref(course.slug);
  const isFreeCourse =
    course.isFree === true || ((course.price || 0) === 0 && (course.priceUSD || 0) === 0);
  ...
  if (isOwned) {
    return (
      <Link href={coursePlayerHref} className={`${baseStyles} ${getVariantStyles()} ${className}`}>
  ...
  return (
    <button onClick={handleClick} className={`${baseStyles} ${getVariantStyles()} ${className}`}>
      {inCart ? (
        <>
        ...
        <>
          <ShoppingCart className="h-5 w-5" />
          {isFreeCourse ? "Obtener gratis" : "Añadir al carrito"}
        </>
      )}
    </button>
  );
