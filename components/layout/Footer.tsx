    ...
    <footer className="bg-footer-bg">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            ...
            <ul className="space-y-3">
              {mainLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-primary hover:text-brand transition-colors text-sm"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          {/* Column 3: Academia de Formación */}
          <div>
            <h3 className="text-footer-title font-medium mb-4 text-sm">
          ...
          <div className="flex flex-col items-center lg:items-end">
            <Link href="/" className="flex flex-col items-center mb-4">
              <Image
                ...
                className="mb-2"
              />
              <h2 className="font-paciencia text-brand text-2xl leading-none text-center">
                ENERGIA Y DIVINIDAD
              </h2>
            </Link>
        ...
        <div className="mt-12 pt-8 border-t border-primary/10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-sm text-primary">
              © 2025 energiaydivinidad.com Todos los derechos reservados
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              {legalLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm text-primary hover:text-brand transition-colors"
                >
                  {link.label}
                </Link>
              ))}
      ...
      <a
        href="https://wa.me/573000000000?text=Bienvenid@, ¿cómo te puedo ayudar?"
        target="_blank"
        rel="noopener noreferrer"
