...
function getAvailableDaysText(session: typeof DEFAULT_SESSION): string {
  ...
  const withIndex = availableDays
    .map((d) => ({ day: d, index: allDays.indexOf(d) }))
    .sort((a, b) => a.index - b.index);
  const sortedDays = withIndex.map((x) => x.day);
  const indices = withIndex.map((x) => x.index);
...
export default async function SesionesPage() {
  ...
  const sessionForCalendar = {
    ...
    priceUSD: sessionData.priceUSD || 100,
    priceEUR: sessionData.priceEUR || 100,
  ...
  const timezoneNote =
    bookingSettings?.timezoneNote || "La sesion sera en hora de Colombia (GMT-5)";
