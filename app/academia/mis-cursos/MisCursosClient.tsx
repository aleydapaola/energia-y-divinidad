...
export function MisCursosClient({ courses }: MisCursosClientProps) {
  ...
  const filteredCourses = courses.filter((item) => {
    ...
    switch (filter) {
      case "in_progress":
        return item.progress.completionPercentage > 0 && item.progress.completionPercentage < 100;
      case "completed":
  ...
  const counts = {
    ...
    in_progress: courses.filter(
      (c) => c.progress.completionPercentage > 0 && c.progress.completionPercentage < 100
    ).length,
    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 sm:pb-0">
            {[
              ...
              >
                {option.label}
                <span className="ml-1.5 opacity-70">({counts[option.value as FilterType]})</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {sortedCourses.length > 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          <p className="text-gray-500 font-dm-sans">No hay cursos que coincidan con tu búsqueda</p>
          <button
            onClick={() => {
              setFilter("all");
              setSearchQuery("");
