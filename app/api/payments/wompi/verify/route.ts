...
export async function GET(request: NextRequest) {
  ...
  if (!transactionId || !reference) {
    return NextResponse.json({ error: "Se requiere transactionId y ref" }, { status: 400 });
  }
  ...
  try {
    ...
    // También repara órdenes de membresía que quedaron COMPLETED sin Subscription
    // por un fallo posterior al cobro.
    if (
      transaction.status === "APPROVED" &&
      (order.paymentStatus !== "COMPLETED" ||
        (order.orderType === "MEMBERSHIP" && !hasMembershipSubscription))
    ) {
