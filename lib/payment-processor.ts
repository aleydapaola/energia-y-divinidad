...
async function createSessionBookingFromOrder(
  ...
  if (isPack) {
    ...
    console.log(
      `[PAYMENT-PROCESSOR] Sesión individual confirmada: ${booking.id} - Fecha: ${scheduledAt}`
    );
  }
...
async function createCourseEntitlementsFromOrder(
  ...
  if (!courseIds || courseIds.length === 0) {
    console.error(
      `[PAYMENT-PROCESSOR] No se encontraron courseIds en metadata para orden ${order.id}`
    );
    return [];
  }
...
async function sendConfirmationEmail(
  ...
  if (!customerEmail) {
    console.warn(
      `[PAYMENT-PROCESSOR] No se encontró email para orden ${order.id}, no se envía confirmación`
    );
    return;
  }

  try {
    const isSession = order.orderType === "SESSION";
    const sessionDate =
      isSession && metadata.scheduledAt ? new Date(metadata.scheduledAt as string) : undefined;
    const meetingLink = isSession ? await getSessionMeetingLink() : undefined;
...
async function findOrCreateUserForGuest(
  ...
  if (existingUser) {
    console.log(
      `[PAYMENT-PROCESSOR] Usuario existente encontrado para guest checkout: ${existingUser.id} (${email})`
    );
...
async function sendAdminSaleNotification(
  ...
  if (!customerEmail) {
    console.warn(
      `[PAYMENT-PROCESSOR] No se encontró email para notificación admin de orden ${order.id}`
    );
    return;
  }

  // Mapear orderType a saleType
  const saleTypeMap: Record<
    string,
    "SESSION" | "SESSION_PACK" | "MEMBERSHIP" | "EVENT" | "COURSE" | "PREMIUM_CONTENT" | "PRODUCT"
  > = {
  ...
  try {
    await sendAdminNotificationEmail({
      ...
      membershipInterval:
        saleType === "MEMBERSHIP" ? metadata.billingInterval || "monthly" : undefined,
      eventDate:
        saleType === "EVENT" && metadata.scheduledAt ? new Date(metadata.scheduledAt) : undefined,
