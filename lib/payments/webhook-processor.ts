...
function generateEventId(
  ...
  const checksum = (verificationResult.rawData as { signature?: { checksum?: string } } | undefined)
    ?.signature?.checksum;
  if (checksum) {
    return `${provider}_${checksum}`;
  }

  const status = verificationResult.status || "unknown";
  return `${provider}_${transactionId}_${status}`;
}
...
export async function processPaymentWebhook(
  ...
  if (!verificationResult.valid) {
    console.error(
      `[WEBHOOK/${gatewayName.toUpperCase()}] Invalid webhook:`,
