...
export async function getDiscountCodeByCode(code: string): Promise<SanityDiscountCode | null> {
  const discountCode = await client.fetch<SanityDiscountCode | null>(DISCOUNT_CODE_BY_CODE_QUERY, {
...
export async function validateDiscountCode(params: {
  code: string;
  userId: string;
  productType: "session" | "pack" | "event" | "course" | "membership";
  courseIds?: string[];
  amount: number;
  currency: "COP" | "USD";
}): Promise<DiscountValidationResult> {
  const { code, userId: _userId, productType, courseIds = [], amount, currency } = params;
  ...
  // 5. Check course restriction (only applies when product is a course)
  if (discountCode.appliesToCourses && discountCode.appliesToCourses.length > 0) {
    if (productType !== "course") {
      return { valid: false, error: "Este código solo aplica para cursos" };
    }
