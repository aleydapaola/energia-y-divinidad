...
import { removeBookingFromGoogleCalendar } from "@/lib/google-calendar";
...
export async function cancelBooking(options: CancellationOptions): Promise<CancellationResult> {
  ...
  try {
    ...
    if (!isAdmin && !skipTimeRestriction && booking.scheduledAt) {
      ...
      if (hoursUntilBooking < 24) {
        return {
          success: false,
          error:
            "Las cancelaciones deben hacerse con al menos 24 horas de anticipación. Contacta con Aleyda para casos especiales.",
          errorCode: "TOO_LATE",
        };
      }
    }
    ...
    if (booking.bookingType === "EVENT") {
      try {
        ...
        if (seatsReleased > 0) {
          ...
          console.log(
            `[CANCELLATION] ${seatsReleased} asiento(s) liberado(s) para booking ${bookingId}`
          );
        }
      } catch (releaseError) {
    ...
    const cancellationReason =
      reason || (isAdmin ? "Cancelada por administrador" : "Cancelada por el cliente");
    ...
    if (booking.bookingType === "SESSION_1_ON_1") {
    ...
    if (booking.bookingType === "EVENT" && refunded.seatsReleased) {
      ...
      message =
        "Sesión cancelada. La sesión ha sido devuelta a tu pack y tu crédito ha sido reembolsado.";
