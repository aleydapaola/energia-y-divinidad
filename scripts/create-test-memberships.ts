...
const membershipTiers: any[] = [
  // NIVEL 1: Esencia - Plan básico (rosa/magenta)
  {
    ...
    description: [
      createBlock(
        "El primer paso hacia tu transformación espiritual. Accede a meditaciones guiadas, contenido exclusivo y una comunidad de apoyo.",
        "d1"
      ),
    ],
    color: "#C77DBA",
  ...
  // NIVEL 2: Divinidad - Plan premium (azul profundo)
  {
    ...
    description: [
      createBlock(
        "La experiencia más completa y personalizada. Incluye sesiones 1:1 mensuales, acceso total a la Academia y los mayores descuentos. Para quienes buscan un acompañamiento profundo.",
        "d1"
      ),
    ],
    color: "#5C4D9B",
    ...
    features: [
      ...
      createFeature(
        "1 sesión 1:1 mensual",
        "Sesión de canalización individual incluida",
        true,
        "f2"
      ),
      createFeature(
        "Todos los cursos",
        "Acceso completo a la Academia sin costo adicional",
        true,
        "f3"
      ),
      createFeature(
        "30% descuento en sesiones extra",
        "El mayor descuento en sesiones adicionales",
        true,
        "f4"
      ),
...
async function createTestMemberships() {
  ...
  try {
    for (const tier of membershipTiers) {
      ...
      const badge =
        tier.popularityBadge === "popular"
          ? "🔥 Más Popular"
          : tier.popularityBadge === "best_value"
            ? "💎 Mejor Valor"
            : "";

      console.log(`     ✅ Creado - Nivel ${tier.tierLevel}`);
      console.log(
        `        Precio mensual: $${tier.pricing.monthlyPrice?.toLocaleString("es-CO")} COP / $${tier.pricing.monthlyPriceUSD} USD`
      );
      console.log(
        `        Precio anual: $${tier.pricing.yearlyPrice?.toLocaleString("es-CO")} COP / $${tier.pricing.yearlyPriceUSD} USD`
      );
      if (badge) {
